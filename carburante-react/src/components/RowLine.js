    ...
    if (!isSelf) {
        ...
        return (
            <tr key={index} style={{ textAlign: 'center', verticalAlign: 'middle' }}>
                <td className="text-capitalize text-start">[{row.brand}]<span className="text-capitalize fw-light"> {row.name} ({((fuelToUse.isSelf) ? (<span className="text-info text-opacity-75">Self</span>) : (<span className="text-secondary text-opacity-75">Servito</span>))})</span></td>
        ...
        return (
            <tr key={index} style={{ textAlign: 'center', verticalAlign: 'middle' }}>
                <td className="text-capitalize text-start">[{row.brand}]<span className="text-capitalize fw-light"> {row.name}</span></td>
